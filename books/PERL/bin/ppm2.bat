...
sub exec_command
{
    ...
    # verify
        ...
        return verify_packages("packages" => \@ARGV, "location" => $location, 
	...
	my $status = 0;
        foreach(keys %summary) {
            print "Refreshing $_\n";
            if (!InstallPackage("package" => $_)) {
                print "Error installing package '$_': $PPM::PPMERR\n";
		++$status;
            }
        }
	return $status;
...
sub set
{
    ...
    unless ($option) {
        ...
        print "Packages will be built under: $options{'BUILDDIR'}\n" 
            if ($options{'BUILDDIR'});
        print "HTML documentation will " . ($options{'REBUILDHTML'} ? "" : "not ") .
            "be rebuilt after installing each package.\n";
         return;
    }
    ...
    if (command($option, "r|epository")) {
        ...
        elsif (command($option, "|rebuildhtml")) {
            $options{'REBUILDHTML'} = defined $value ? 
                ($value != 0) : ($options{'REBUILDHTML'} ? 0 : 1);
            print "HTML documentation will " . 
                ($options{'REBUILDHTML'} ? "" : "not ") . "be rebuilt after installing packages.\n";
        }
...
sub verify_packages
{
    ...
    while ($package) {
        ...
        if (defined $status) {
            ...
            # Hack: no warning if its on of the core PPM 
            # modules (hardcoded below :)
            my $no=0;
            foreach my $corepackage (
            "Compress-Zlib","Archive-Tar","Digest-MD5","File-CounterFile",
            "Font-AFM","HTML-Parser","HTML-Tree","MIME-Base64","URI",
            "libwww-perl","XML-Parser","SOAP-Lite","PPM","libnet",
            "libwin32","HTML-Tagset"
            )
            {
                if ($corepackage eq $package) {
                    $no=1;
                    last;
                }
            }
	    unless ($no) {
		++$failed;
		print "While verifying package '$package': $PPM::PPMERR\n";
	    }
        }
...
<PPMCONFIG>
    ...
    <OPTIONS REBUILDHTML="0" BUILDDIR="$ENV{'TEMP'}" CLEAN="1" CONFIRM="1" DOWNLOADSTATUS="16384" FORCEINSTALL="1" IGNORECASE="1" MORE="0" ROOT="$PerlDir" TRACE="0" TRACEFILE="PPM.LOG" VERBOSE="1" />
    ...
    <PPMPRECIOUS>Compress-Zlib;Archive-Tar;Digest-MD5;File-CounterFile;Font-AFM;HTML-Parser;HTML-Tree;MIME-Base64;URI;libwww-perl;XML-Parser;SOAP-Lite;PPM;libnet;libwin32</PPMPRECIOUS>
</PPMCONFIG>
EOF
}
...
=item ppm2 verify --location=P:\PACKAGES --upgrade foo
...
=item ppm2 verify --upgrade --force
...
=item ppm2 search --location=http://ppm.ActiveState.com/PPMpackages/5.6

Displays the packages with PPD files available at the specified location.

=item ppm2 search --location=P:\PACKAGES --author ActiveState
